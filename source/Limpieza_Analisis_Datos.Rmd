...
visualize_distribution("sex")
visualize_distribution("chest_pain_type")
...
par(mfrow=c(2, 3))
visualize_distribution("rest_ecg_type")
visualize_distribution("max_heart_rate_achieved")
visualize_distribution("exercise_induced_angina")
...
```{r}
outliers_treatment <- function(variable, threshold){
  
  # Número de outliers que superan el valor determinado en la variable tratada
  num_outliers = nrow(heart_data[heart_data$variable] > threshold)
  
  # Número de NA's en la variable tratada
  num_nas = sum(is.na(heart_data[heart_data$variable]))
  
  # Se guardan para luego comprobar que se han imputado bien
  atipicos_dataset = heart_data %>% select(variable) %>% filter(variable > threshold) %>% head(5)

  # Se substituyen esos valores atípicos por el valor NA
  heart_data = heart_data %>% mutate(variable = ifelse(variable > threshold, NA, variable)) 
  
  # Numero de NA's en la variable tratada
  num_nas_final = sum(is.na(heart_data$variable))

  # Se muestran los resultados
  print(paste("Número de outliers:", num_outliers))
  print(paste("Número de NA's iniciales:", num_nas))
  print(paste("Número de NA's finales:", num_nas_final))
}
```

```{r}
cholesterol_clean <- outliers_treatment("cholesterol", 500)
# Una vez realizada la modificación de estos valores atípicos por valores NA 
# podemos ver que se ha realizado con éxito este procedimiento
heart_data %>% select("cholesterol") %>%filter(is.na("cholesterol"))
```

```{r}
#max_heart_rate_achieved_clean <- outliers_treatment("max_heart_rate_achieved", 300, 80)
```

```{r}
#resting_blood_pressure_clean <- outliers_treatment("resting_blood_pressure", 190)
```

```{r}
#st_depression_clean <- outliers_treatment("st_depression", 5.5)
```

============
Eliminar del conjunto de datos: 

- cholesterol:  564
- max_heart_rate_achieved: 71 
- resting_blood_pressure: 192, 200
- st_depression: 5.6, 6.2
