...
variable_tratada = heart_data$cholesterol
limit = 500

# Número de outliers que superan el valor establecido en la variable tratada
num_outliers_var = nrow(heart_data[variable_tratada >limit,])

# Número de NA's en la variable tratada
num_nas_variable_inicio = sum(is.na(variable_tratada))

# Se substituyen esos valores atipicos por el valor NA
heart_data = heart_data %>% mutate(cholesterol = ifelse(cholesterol > limit, NA, cholesterol))

# Numero de NA's final después del tratado en la variable
num_nas_variable_final = sum(is.na(variable_tratada))
...
```{r}
# Parámetros iniciales
variable_tratada = heart_data$resting_blood_pressure
limit = 190
...
# Se substituyen esos valores atipicos por el valor NA
heart_data = heart_data %>% mutate(resting_blood_pressure = ifelse(resting_blood_pressure > limit, NA, resting_blood_pressure))
...
```{r}
# Parámetros iniciales
variable_tratada = heart_data$st_depression
limit = 6
...
# Se substituyen esos valores atipicos por el valor NA
heart_data = heart_data %>% mutate(st_depression = ifelse(st_depression > limit, NA, st_depression))
