...
– *cp*: Identifica el tipo de dolor en el pecho (0 = angina típica, 1 = angina atípica, 2 = dolor no anginoso, 3 = asintomático).

– *fbs*: Indica si el nivel de azúcar en sangre en ayunas es mayor a 120 mg/dl (1 = verdadero, 0 = falso).

– *restecg*: Muestra los resultados electrocardiográficos en reposo (0 = normal, 1 = anomalía de onda ST-T (inversiones de onda T y/o elevación o depresión ST de > 0,05 mV), 2 = hipertrofia ventricular izquierda probable o definida por los criterios de Estes).

– *exng*: Indica si la angina ha sido inducida por el ejercicio (1 = sí, 0 = no).

– *oldpeak*: Señala la depresión ST inducida por el ejercicio en relación con el descanso.

– *slp*: Muestra la pendiente del segmento ST de ejercicio máximo (0 = inclinación hacia abajo, 1 = plano, 2 = inclinación hacia arriba).

– *caa*: Indica el número de buques principales (0, 1, 2, 3).

– *thall*: Señala el ratio de un trastorno sanguíneo llamado talasemia (0 = no tiene, 1 = defecto fijo (sin flujo sanguíneo en alguna parte del corazón), 2 = flujo sanguíneo normal, 3 = defecto reversible (se observa un flujo sanguíneo, pero no es normal)).

– *output*: Indica si el paciente sufre un ataque al corazón o no (0 = No, 1 = Sí). 
...
colnames(heart_data)
#data_to_show <- subset(heart_data, select=c("age", "chol", "thalachh", "trtbps", "oldpeak"))
```

```{r}
# Valores extremos en formato boxplot de la variable age
boxplot(data_to_show$age, main = "Valores extremos de de la variable age", ylab="Valor", col = "lightblue", horizontal = FALSE)
```

```{r}
# Valores extremos en formato boxplot de la variable chol
boxplot(data_to_show$chol, main = "Valores extremos de de la variable chol", ylab="Valor", col = "lightblue", horizontal = FALSE)
```

```{r}
# Valores extremos en formato boxplot de la variable thalachh
boxplot(data_to_show$thalachh, main = "Valores extremos de de la variable thalachh", ylab="Valor", col = "lightblue", horizontal = FALSE)
```

```{r}
# Valores extremos en formato boxplot de la variable trtbps
boxplot(data_to_show$trtbps, main = "Valores extremos de de la variable trtbps", ylab="Valor", col = "lightblue", horizontal = FALSE)
```


```{r}
# Valores extremos en formato boxplot de la variable oldpeak
boxplot(data_to_show$oldpeak, main = "Valores extremos de de la variable oldpeak", ylab="Valor", col = "lightblue", horizontal = FALSE)
```
