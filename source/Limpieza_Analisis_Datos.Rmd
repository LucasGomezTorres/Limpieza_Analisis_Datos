...
````{r}
# Selección de la información a gestionar
row_heart_data <- read.csv("heart.csv", header = TRUE)
...
boxplot(data_selected$age, main = "Valores extremos de de la variable age", 
        ylab="Valor", col = "lightblue", horizontal = FALSE)
```


```{r}
outlier_info <- function(var) {
  # Valores extremos en formato boxplot de la variable
  box = boxplot(var, main = "Valores extremos de la variable", 
        ylab="Valor", col = "lightblue", horizontal = FALSE)
  ...
  if (length(outliers) == 0) {
    ...
    cat("Máximos y mínimos de los valores extremos identificados:", range(outliers))
    }
  }
```

```{r}
outlier_info(data_selected$age)
```

```{r}
outlier_info(data_selected$chol)
```

```{r}
outlier_info(data_selected$thalachh)
```

```{r}
outlier_info(data_selected$trtbps)
```

```{r}
outlier_info(data_selected$oldpeak)
```



# Vemos el mínimo y el máximo de los anteriores valores atípicos
range(boxplot.stats(dataset_gpa$sat)$out)
